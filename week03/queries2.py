...
def filter(file_name,**kwargs):
    ...
    for i in range(len(array)):
        ...
        if n > 6:
            ...
            # print('---------')
            #print(array[i][2:2+num_of_additional_cols+1] )
    ...
    if len(kwargs)==2:
        ...
        for key,val in kwargs.items():
            ...
            if key=='full_name' and count==0:
                for i in range(len(array)):
                    if array[i][0]==val:
                        result_array[len(result_array):]=[array[i]]
                count+=1
            if key=='full_name__starswith' and count==0:
                for i in range(len(array)):
                    k=len(val)
                    if array[i][0][:k]==val:
                        result_array[len(result_array):]=[array[i]]
                    count+=1
            if key=='email__contains' and count==0:
                for i in range(len(array)):
                    if val in array[i][3]:
                        result_array[len(result_array):]=[array[i]]
                    count+=1
            ...
            #         if result_array[i][0]==val:
            #             result_array[len(result_array):]=[result_array[i]]
            # if key=='full_name__starswith' and count>0:
            #     for i in range(len(result_array)):
            #         k=len(val)
            #         if result_array[i][0][:k]==val:
            #            # print(' '.join(array[i]))
            #             result_array[len(result_array):]=[result_array[i]]
            # if key=='email__contains' and count>0:
            #     for i in range(len(result_array)):
            #         if val in result_array[i][3]:
            #             result_array[len(result_array):]=[result_array[i]]
            if key=='favourite_color' and count>0:
                print('fewr')
                result_array=func(result_array,1,val)
