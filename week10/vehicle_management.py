...
def update_repair_hour(_id,to_change,val):
    connection = sqlite3.connect('vehicle_management.db')
    cursor = connection.cursor()
...
def show_repair_hour(id):
    ...
    if choice=='2':
        ...
        print('New Bill:')
        val=input()
    update_repair_hour(id,to_change,val)
...
def save_repair_hour(hour_id):
    ...
    cursor.execute("SELECT id,make,model,register_number FROM Vehicle;")
...
def main():
    ...
    while flag:
        ...
        while inner_flag:
            ...
            print('6 add_vehicle')
            print('7 update_vehicle <vehicle_id>')
            print('8 delete_vehicle <vehicle_id>')
            print('exit')
            command=input()
            if command=='1':
                list_all_free_hours()
