#KOGATO GLEDAME I S MINUSI
#eval_derivative FIND namira 1voto povtorenieq sledovatelno v posledniq primer ne se poluchava
...
class Derivative:
    ...
    def eval_derivative(self):
        ...
        xpow=self.lst[0][1]-1
        ind=self.expr.find(str(self.lst[0][0]))
        #print(coeff,xpow)
        if xpow==1:
            if self.expr[ind-1]=='-':
                res='-'+str(coeff)+'*x'
        ...
        for i in range(1,len(self.lst)):
            ...
            ind=self.expr.find(str(self.lst[i][0]))
            #print(coeff,xpow)
            if xpow==1:
                if self.expr[ind-1]=='+':
                    res+='+'+str(coeff)+'*x'
                elif self.expr[ind-1]=='-':
                    res+='-'+str(coeff)+'*x'
                continue
            elif xpow==0:
                if self.expr[ind-1]=='+':
